...
if(paste0("events_", aktuelt_year, ".csv") %in% data_filer){
  ...
  aktuelt_events <- aktuelt_events %>% mutate(online_join_password = NA_character_)  # udfordringer med inkonsistent numeric vs character password.
  past_events <- past_events %>% mutate(online_join_password = NA_character_ )
