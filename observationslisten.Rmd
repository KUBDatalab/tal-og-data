...
til_tabel <- data %>% 
  ...
  mutate(andel = seats_taken/seats*100) %>% 
  arrange(start) 

til_tabel$andel <- color_bar(csscolor(gradient(as.numeric(til_tabel$andel), "red", "green")))(til_tabel$andel)
til_tabel %>% kable(escape = F)
```
